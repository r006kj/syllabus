...
type PlanMode = 'normal' | 'parciales' | 'finales'
const PLAN_MODES: Record<PlanMode, { label: string; description: string; icon: string; color: string; border: string }> = {
  normal:    { label: 'Estudio normal', description: 'Sesiones diarias distribuidas',     icon: 'menu_book',   color: 'bg-blue-500',   border: 'border-blue-500'   },
  parciales: { label: 'Parciales',      description: 'Intensivo para exámenes parciales', icon: 'history_edu', color: 'bg-amber-500',  border: 'border-amber-500'  },
  finales:   { label: 'Finales',        description: 'Máxima intensidad para finales',    icon: 'school',      color: 'bg-red-500',    border: 'border-red-500'    },
}
...
const PREFS_KEY = 'study_prefs_v1'
...
const loadPrefs = (): StudyPrefs => {
  try { const r = localStorage.getItem(PREFS_KEY); if (r) return { ...DEFAULT_PREFS, ...JSON.parse(r) } } catch { /* ignore */ }
  return DEFAULT_PREFS
}
const savePrefs = (p: StudyPrefs) => localStorage.setItem(PREFS_KEY, JSON.stringify(p))
...
const BlockModal = ({ block, onSave, onDelete, onClose }: {
  ...
  const [form, setForm] = useState<Partial<ScheduleBlock>>({
    day_of_week: 0, start_time: '08:00', end_time: '10:00', course_name: '', location: '', attendance_required: true, ...block
  })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-[#1a1a1a] rounded-2xl shadow-2xl w-full max-w-sm p-6 border border-white/10">
        ...
        <div className="flex flex-col gap-3">
          {[{ label: 'Curso', key: 'course_name', placeholder: 'Ej: Cálculo III' }, { label: 'Ubicación', key: 'location', placeholder: 'Aula 301 (opcional)' }].map(f => (
            <div key={f.key}>
              <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest mb-1 block">{f.label}</label>
              <input className="w-full rounded-xl border border-white/10 bg-white/5 text-sm text-white px-3 py-2 focus:outline-none focus:border-pink-500 placeholder-white/20"
          ...
          <div>
            ...
            <select className="w-full rounded-xl border border-white/10 bg-white/5 text-sm text-white px-3 py-2 focus:outline-none focus:border-pink-500"
              value={form.day_of_week ?? 0} onChange={e => setForm(p => ({ ...p, day_of_week: Number(e.target.value) }))}>
          ...
          <div className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 mt-1">
            ...
            <button type="button" onClick={() => setForm(p => ({ ...p, attendance_required: !p.attendance_required }))}
              className={`w-11 h-6 rounded-full relative transition-colors flex items-center px-1 flex-shrink-0 ${form.attendance_required ? 'bg-pink-500' : 'bg-white/15'}`}>
              <span className={`w-4 h-4 rounded-full bg-white transition-transform ${form.attendance_required ? 'translate-x-5' : 'translate-x-0'}`} />
            </button>
          </div>
        </div>
        <div className="flex gap-2 mt-5">
          <button onClick={async () => { setSaving(true); try { await onSave(form) } finally { setSaving(false) } }} disabled={saving}
          ...
          {onDelete && (
            <button onClick={onDelete} className="px-4 rounded-xl border border-red-500/30 text-red-400 text-sm font-bold hover:bg-red-500/10 transition-colors">
              Eliminar
            </button>
          )}
        </div>
...
const SessionModal = ({ session, tasks, onSave, onDelete, onClose }: {
  session: Partial<StudySession>; tasks: Task[]
  ...
  const toggleTask = (id: string) =>
    setForm(p => ({ ...p, _selected_task_ids: (p._selected_task_ids ?? []).includes(id)
      ? (p._selected_task_ids ?? []).filter(x => x !== id) : [...(p._selected_task_ids ?? []), id] }))

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className={`bg-[#1a1a1a] rounded-2xl shadow-2xl w-full max-w-sm border-2 ${colors.border} overflow-hidden`}>
        ...
        <div className="flex border-b border-white/10 px-6">
          {(['info','tasks'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`pb-2.5 mr-5 text-xs font-bold border-b-2 transition-colors ${tab === t ? `${colors.border} text-white` : 'border-transparent text-white/40 hover:text-white/60'}`}>
        ...
        <div className="px-6 py-4 max-h-[60vh] overflow-y-auto">
          {tab === 'info' && (
            <div className="flex flex-col gap-3">
              <div>
                ...
                <input className={`w-full rounded-xl border ${colors.border}/40 bg-white/5 text-sm text-white px-3 py-2 focus:outline-none placeholder-white/20`}
                  value={form._title ?? ''} onChange={e => setForm(p => ({ ...p, _title: e.target.value }))} placeholder="Ej: Repaso Cálculo" />
              </div>
              <div>
                <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest mb-1 block">Curso</label>
                <input className={`w-full rounded-xl border ${colors.border}/40 bg-white/5 text-sm text-white px-3 py-2 focus:outline-none placeholder-white/20`}
                  value={form._course_name ?? ''} onChange={e => setForm(p => ({ ...p, _course_name: e.target.value }))} placeholder="Curso relacionado" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                {(['start_time','end_time'] as const).map((k, i) => (
                  <div key={k}>
                    <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest mb-1 block">{i === 0 ? 'Inicio' : 'Fin'}</label>
                    <input type="datetime-local" className={`w-full rounded-xl border ${colors.border}/40 bg-white/5 text-xs text-white px-2 py-2 focus:outline-none`}
                      value={form[k]?.slice(0,16) ?? ''} onChange={e => setForm(p => ({ ...p, [k]: e.target.value }))} />
                  </div>
                ))}
              </div>
              <div>
                <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest mb-1 block">Prioridad</label>
                <select className={`w-full rounded-xl border ${colors.border} ${colors.bg} text-sm text-white px-3 py-2 focus:outline-none`}
                  value={form.priority ?? 'medium'} onChange={e => setForm(p => ({ ...p, priority: e.target.value }))}>
          ...
          {tab === 'tasks' && (
            <div className="flex flex-col gap-2">
              {pendingTasks.length === 0 && <p className="text-xs text-white/30 text-center py-6">No hay tareas pendientes.</p>}
              {pendingTasks.map(task => {
                ...
                return (
                  <button key={task.id} onClick={() => toggleTask(task.id)}
                    className={`w-full text-left rounded-xl border px-3 py-2.5 transition-all ${selected ? `${colors.border} ${colors.bg}` : 'border-white/10 bg-white/5 hover:border-white/20'}`}>
                    ...
                    {task.courses?.name && <p className="text-[10px] text-white/30 mt-1 ml-6">{task.courses.name}</p>}
                  </button>
                )
              })}
            </div>
...
const PrefsModal = ({ prefs, onSave, onClose }: {
  prefs: StudyPrefs; onSave: (p: StudyPrefs) => void; onClose: () => void
}) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-[#1a1a1a] rounded-2xl shadow-2xl w-full max-w-sm p-6 border border-white/10">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {([['preferredStart','Hora inicio'], ['preferredEnd','Hora fin']] as const).map(([k, l]) => (
              <div key={k}>
              ...
              {DAY_LABELS.map((d, i) => (
                <button key={i} onClick={() => toggleDay(i)}
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-colors ${form.studyDays.includes(i)
                    ? 'bg-pink-500 text-white' : 'bg-white/5 border border-white/10 text-white/40 hover:text-white/70'}`}>
                  {d}
                </button>
              ))}
            </div>
...
const GeneratePlanModal = ({ tasks, prefs, scheduleBlocks, semesterStart, generating, onGenerate, onClose }: {
  tasks: Task[]
  prefs: StudyPrefs
  scheduleBlocks: ScheduleBlock[]
  semesterStart: string | null
  generating: boolean
  onGenerate: (mode: PlanMode, taskIds: string[]) => Promise<void>
  onClose: () => void
}) => {
  ...
  const toggleAll = () =>
    setSelectedIds(p => p.length === pendingTasks.length ? [] : pendingTasks.map(t => t.id))
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-[#1a1a1a] rounded-2xl shadow-2xl w-full max-w-md border border-white/10 overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-white/8">
          <div>
            <h2 className="text-sm font-bold text-white">Generar plan con IA</h2>
            <p className="text-[10px] text-white/40 mt-0.5">La IA usará tu horario, preferencias y tareas seleccionadas</p>
          </div>
          <button onClick={onClose} className="text-white/40 hover:text-white transition-colors">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6L6 18M6 6l12 12"/></svg>
          </button>
        ...
        <div className="p-5 flex flex-col gap-5 max-h-[70vh] overflow-y-auto">
          ...
          {/* Study mode — 3 buttons */}
          <div>
            <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest mb-2">Tipo de estudio</p>
            <div className="grid grid-cols-3 gap-2">
              {(Object.entries(PLAN_MODES) as [PlanMode, typeof PLAN_MODES.normal][]).map(([m, info]) => (
                <button key={m} onClick={() => setMode(m)}
                  className={`rounded-xl p-3 text-left border-2 transition-all ${mode === m
                    ? `${info.border} ${info.color}/20`
                    : 'border-white/10 bg-white/5 hover:border-white/20'}`}>
                  <span className={`material-symbols-outlined text-[20px] mb-1 block ${mode === m ? 'text-white' : 'text-white/30'}`}>{info.icon}</span>
                  <p className={`text-xs font-bold leading-tight ${mode === m ? 'text-white' : 'text-white/40'}`}>{info.label}</p>
                  <p className={`text-[9px] mt-0.5 leading-tight ${mode === m ? 'text-white/60' : 'text-white/20'}`}>{info.description}</p>
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest">
                Tareas a incluir <span className="text-pink-400">({selectedIds.length}/{pendingTasks.length})</span>
              </p>
              <button onClick={toggleAll} className="text-[10px] text-pink-400 hover:text-pink-300 font-bold transition-colors">
                {selectedIds.length === pendingTasks.length ? 'Deseleccionar todo' : 'Seleccionar todo'}
              </button>
            </div>
            {pendingTasks.length === 0 ? (
              ...
              <div className="flex flex-col gap-1.5 max-h-48 overflow-y-auto">
                {pendingTasks.map(task => {
                  ...
                  const due     = task.due_date ? new Date(task.due_date) : null
                  const overdue = due && due < new Date()
                  const soon    = due && !overdue && (due.getTime() - nowMs) < 7 * 86400000
                  return (
                    <button key={task.id} onClick={() => toggle(task.id)}
                      className={`w-full text-left rounded-xl border px-3 py-2 transition-all ${checked
                        ? 'border-pink-500/60 bg-pink-500/10'
                        : 'border-white/8 bg-white/[0.03] hover:border-white/15'}`}>
                      <div className="flex items-center gap-2.5">
                        <div className={`w-4 h-4 rounded-md border-2 flex items-center justify-center flex-shrink-0 ${checked ? 'border-pink-500 bg-pink-500' : 'border-white/20'}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {task.courses?.name && <p className="text-[9px] text-white/30 truncate">{task.courses.name}</p>}
                        </div>
                        {due && (
                          <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-lg flex-shrink-0 ${overdue ? 'bg-red-500/20 text-red-400' : soon ? 'bg-amber-500/20 text-amber-400' : 'bg-white/5 text-white/30'}`}>
                            {due.toLocaleDateString('es-PE', { day: 'numeric', month: 'short' })}
                          </span>
                        )}
                      </div>
          ...
          {/* Context preview — what AI receives */}
          <div className="rounded-xl border border-white/8 bg-white/[0.02] p-3">
            <p className="text-[10px] font-bold text-white/30 uppercase tracking-widest mb-2">La IA recibirá</p>
            <div className="flex flex-col gap-1">
              {[
                { icon: 'calendar_view_week', text: `${scheduleBlocks.length} bloques de clases en tu horario` },
                { icon: 'schedule',          text: `${prefs.dailyHours}h/día · ${prefs.preferredStart}–${prefs.preferredEnd}` },
                { icon: 'coffee',            text: `Descansos de ${prefs.breakMin}min entre sesiones` },
                { icon: 'today',             text: `Días disponibles: ${DAY_LABELS.filter((_, i) => prefs.studyDays.includes(i)).join(', ')}` },
                ...(semesterStart ? [{ icon: 'school', text: `Semestre desde ${new Date(semesterStart).toLocaleDateString('es-PE', { day: 'numeric', month: 'short' })}` }] : []),
              ].map((item, i) => (
                <div key={i} className="flex items-center gap-2">
                  <span className="material-symbols-outlined text-white/20 text-[13px]">{item.icon}</span>
        ...
        <div className="px-5 pb-5">
          <button
            ...
            className={`w-full py-3 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2
              ${PLAN_MODES[mode].color} hover:brightness-110 disabled:opacity-50 text-white`}>
            ...
            {generating ? 'Generando...' : `Generar plan · ${PLAN_MODES[mode].label}`}
          </button>
        </div>
      </div>
    </div>
...
export const Calendar = () => {
  ...
  const [dropPreview, setDropPreview] = useState<{ dayIdx: number; startMin: number } | null>(null)
  ...
  useEffect(() => {
    if (scrollRef.current) scrollRef.current.scrollTop = (START_HOUR_DEFAULT - startHour) * HOUR_HEIGHT
  }, [extended, startHour])

  // ── CRUD ────────────────────────────────────────────────────────────────────

  const saveBlock = async (form: Partial<ScheduleBlock>) => {
    const payload = { day_of_week: form.day_of_week, start_time: form.start_time, end_time: form.end_time,
      course_name: form.course_name ?? null, location: form.location ?? null, attendance_required: form.attendance_required ?? true }
    if (form.id) await api.patch(`/schedule/blocks/${form.id}`, payload)
    else          await api.post('/schedule/blocks', payload)
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (pdfs.length > 0 && images.length > 0) {
      alert('Sube solo imágenes o solo un PDF, no ambos a la vez.')
    ...
    if (pdfs.length > 1) {
      alert('Solo puedes subir 1 PDF.')
      e.target.value = ''; return
    }
    if (pdfs.length === 1) {
      const pages = await countPdfPages(pdfs[0])
      if (pages > 2) {
        alert('El PDF no puede tener más de 2 páginas.')
        e.target.value = ''; return
      }
    }
    ...
    files.forEach(f => fd.append(f.type === 'application/pdf' ? 'pdf' : 'image', f))
    try { await api.post('/schedule/upload', fd, { headers: { 'Content-Type': 'multipart/form-data' } }); await fetchAll() }
    catch (err) { console.error('Upload error', err) }
    finally { setUploading(false); e.target.value = '' }
  }

  const generatePlan = async (mode: PlanMode, taskIds: string[]) => {
    setGenerating(true)
    try {
      await api.post('/planner/generate', {
        ...
        task_ids: taskIds,
        study_prefs: prefs,
        schedule_blocks: scheduleBlocks.map(b => ({
          day_of_week: b.day_of_week,
          start_time:  b.start_time,
          end_time:    b.end_time,
          course_name: b.course_name,
        })),
        semester_start: settings?.semester_start ?? null,
      })
      await fetchAll()
  ...
  const saveSession = async (form: Partial<StudySession>) => {
    ...
    if (form.id) await api.patch(`/planner/study-blocks/${form.id}`, payload)
    else          await api.post('/planner/study-blocks', payload)
    await fetchAll()
  ...
  const blockStyle = (start: string, end: string) => {
    ...
    return { top: `${(s / 60) * HOUR_HEIGHT}px`, height: `${Math.max(((e - s) / 60) * HOUR_HEIGHT, 24)}px` }
  }

  const nowTop = ((today.getHours() * 60 + today.getMinutes() - startHour * 60) / 60) * HOUR_HEIGHT

  const priorityBadge = (p?: string) =>
    p === 'high' ? 'bg-red-500/20 text-red-400' : p === 'low' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'

  // ── Drag helpers ─────────────────────────────────────────────────────────────

  const startBlockDrag = (e: React.DragEvent, block: ScheduleBlock) => {
    const rect      = (e.currentTarget as HTMLElement).getBoundingClientRect()
    const offsetY   = e.clientY - rect.top
    const offsetMin = Math.round((offsetY / HOUR_HEIGHT) * 60)
    const durMin    = timeToMinutes(block.end_time) - timeToMinutes(block.start_time)
    setDragInfo({ type: 'block', id: block.id, durationMin: durMin, offsetMin })
    e.dataTransfer.effectAllowed = 'move'
  }

  const startSessionDrag = (e: React.DragEvent, session: StudySession) => {
    const rect       = (e.currentTarget as HTMLElement).getBoundingClientRect()
    const offsetY    = e.clientY - rect.top
    const offsetMin  = Math.round((offsetY / HOUR_HEIGHT) * 60)
    const startStr   = getTimeStringFromIso(session.start_time)
    const endStr     = getTimeStringFromIso(session.end_time)
    const durMin     = timeToMinutes(endStr) - timeToMinutes(startStr)
    setDragInfo({ type: 'session', id: session.id, durationMin: durMin, offsetMin })
    e.dataTransfer.effectAllowed = 'move'
  }
  ...
  const onColumnDragOver = (e: React.DragEvent, dayIdx: number) => {
    ...
    e.preventDefault()
    const rect   = (e.currentTarget as HTMLElement).getBoundingClientRect()
    const y      = e.clientY - rect.top  // rect already reflects scroll via viewport coords
    const rawMin = (y / HOUR_HEIGHT) * 60 + startHour * 60 - dragInfo.offsetMin
    const snapped   = Math.round(rawMin / 15) * 15
    const clamped   = Math.max(startHour * 60, Math.min(snapped, endHour * 60 - dragInfo.durationMin))
    setDropPreview({ dayIdx, startMin: clamped })
  }

  const onColumnDrop = async (e: React.DragEvent, dayIdx: number) => {
    e.preventDefault()
    if (!dragInfo || !dropPreview || dropPreview.dayIdx !== dayIdx) return

    const endMin = dropPreview.startMin + dragInfo.durationMin

    if (dragInfo.type === 'block') {
      const block = scheduleBlocks.find(b => b.id === dragInfo.id)
      if (block) {
        await api.patch(`/schedule/blocks/${dragInfo.id}`, {
          day_of_week: dayIdx,
          start_time:  minutesToTime(dropPreview.startMin),
          end_time:    minutesToTime(endMin),
          course_name: block.course_name,
          location:    block.location,
          attendance_required: block.attendance_required,
        })
        await fetchAll()
      }
    } else {
      const session = sessions.find(s => s.id === dragInfo.id)
      if (session) {
        const targetDate = new Date(weekDates[dayIdx])
        targetDate.setHours(Math.floor(dropPreview.startMin / 60), dropPreview.startMin % 60, 0, 0)
        const endDate = new Date(targetDate.getTime() + dragInfo.durationMin * 60000)
        await api.patch(`/planner/study-blocks/${dragInfo.id}`, {
          start_time:  targetDate.toISOString(),
          end_time:    endDate.toISOString(),
          priority:    session.priority,
          task_id:     session.task_id ?? null,
          title:       session._title ?? session.tasks?.title,
          course_name: session._course_name,
        })
        await fetchAll()
      }
    }
  ...
  return (
    <div className="flex min-h-screen bg-[#111111] font-body">
              ...
              {weekDates.map((date, i) => {
                ...
                return (
                  <div key={i} className={`flex-1 text-center py-3 border-l border-white/5 ${isToday ? 'bg-pink-500/5' : ''}`}>
                    <div className={`text-[9px] font-bold uppercase tracking-widest ${isToday ? 'text-pink-400' : 'text-white/25'}`}>{DAY_LABELS[i]}</div>
                    <div className={`text-base font-bold leading-tight mt-0.5 ${isToday
                      ...
                      : 'text-white/60'}`}>{date.getDate()}</div>
                  </div>
                )
              })}
            </div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto overflow-x-hidden"
              ...
              <div className="flex" style={{ height: `${totalHours * HOUR_HEIGHT}px` }}>
                ...
                {weekDates.map((date, dayIdx) => {
                  ...
                  const hasDropPreview = dropPreview?.dayIdx === dayIdx

                  return (
                    <div key={dayIdx}
                      className={`flex-1 relative border-l border-white/5 ${isToday ? 'bg-pink-500/[0.02]' : ''} ${hasDropPreview ? 'bg-pink-500/[0.03]' : ''}`}
                    ...
                    >
                      {/* Hour grid lines */}
                      {hours.map(h => (
                        <div key={h} className="absolute left-0 right-0 border-t border-white/5" style={{ top: `${(h - startHour) * HOUR_HEIGHT}px` }} />
                      ))}
                      {hours.slice(0,-1).map(h => (
                        <div key={`h-${h}`} className="absolute left-0 right-0 border-t border-white/[0.02]"
                      ...
                      {/* Drop preview ghost */}
                      {hasDropPreview && dragInfo && (
                        <div className="absolute left-0.5 right-0.5 rounded-lg border-2 border-dashed border-pink-400 bg-pink-500/10 z-30 pointer-events-none"
                          style={blockStyle(minutesToTime(dropPreview!.startMin), minutesToTime(dropPreview!.startMin + dragInfo.durationMin))} />
                      )}
                      ...
                      {dayBlocks.map(block => {
                        ...
                        return (
                          <button key={block.id}
                            draggable
                            onDragStart={e => startBlockDrag(e, block)}
                            onClick={() => !dragInfo && setEditingBlock(block)}
                            className={`absolute left-0.5 right-0.5 rounded-lg border-l-[3px] px-2 py-1 text-left group transition-all z-10 cursor-grab active:cursor-grabbing
                              ${color.bg} ${color.border}
                              ${isDragging ? 'opacity-30' : 'hover:brightness-110'}`}
                            style={{ top: style.top, height: style.height }}>
                            <div className="flex items-center gap-1">
                              {block.attendance_required && (
                                <svg width="8" height="8" viewBox="0 0 24 24" fill="currentColor" className={`${color.text} opacity-60 flex-shrink-0`}><circle cx="12" cy="12" r="10"/></svg>
                              )}
                      ...
                      {daySessions.map(session => {
                        ...
                        const durMin     = timeToMinutes(endStr) - timeToMinutes(startStr)
                        const isDragging = dragInfo?.id === session.id

                        return (
                          <button key={`s-${session.id}`}
                            draggable
                            onDragStart={e => startSessionDrag(e, session)}
                            onClick={() => !dragInfo && setEditingSession(session)}
                            className={`absolute left-0.5 right-0.5 rounded-lg border-l-[3px] px-2 py-1 text-left group transition-all z-10 cursor-grab active:cursor-grabbing
                            ...
                            {durMin >= 50 && (
                              <p className={`text-[9px] opacity-70 truncate mt-0.5 ${colors.text}`}>{startStr}–{endStr}</p>
                            )}
                          </button>
                        )
                      })}
          ...
          <div className="w-[280px] flex-shrink-0 flex flex-col border-l border-white/8 bg-[#161616]">
            ...
            <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2">
              ...
              {sessions.map(session => {
                ...
                const colors    = priorityColor(session.priority)
                const selTitles = (session._selected_task_ids ?? []).map(id => tasks.find(t => t.id === id)?.title).filter(Boolean)

                return (
                  <div key={session.id} className={`rounded-xl border ${colors.border}/30 bg-white/[0.03] hover:${colors.border}/60 transition-all group p-3`}>
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="flex items-center gap-2 min-w-0">
                        <div className={`w-2 h-2 rounded-full flex-shrink-0 ${colors.dot}`} />
                        <p className="text-xs font-bold text-white truncate">{taskTitle}</p>
                      </div>
                      <button onClick={() => setEditingSession(session)} className="opacity-0 group-hover:opacity-100 transition-opacity text-white/30 hover:text-pink-400 flex-shrink-0">
                    ...
                    {selTitles.length > 0 && (
                      <div className="flex flex-wrap gap-1 mb-2 pl-4">
                        {selTitles.map((t, i) => <span key={i} className="text-[9px] bg-white/8 text-white/50 px-2 py-0.5 rounded-lg truncate max-w-[120px]">{t}</span>)}
                      </div>
                    )}
                    <div className="flex items-center justify-between pl-4">
                      <div>
                        {start && <span className="text-[10px] text-white/30 font-mono">{start.toLocaleDateString('es-PE', { weekday: 'short', day: 'numeric', month: 'short' })} · {start.toLocaleTimeString('es-PE', { hour: '2-digit', minute: '2-digit' })}</span>}
                      </div>
                      <div className="flex items-center gap-1.5">
                        {session.priority && <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-lg ${priorityBadge(session.priority)}`}>{session.priority === 'high' ? 'Alta' : session.priority === 'low' ? 'Baja' : 'Media'}</span>}
                        {dur && <span className="text-[10px] font-bold text-pink-400 bg-pink-500/15 px-2 py-0.5 rounded-lg">{dur}</span>}
                      </div>
                    </div>
                  </div>
                )
      ...
      {editingBlock !== null && (
        <BlockModal block={editingBlock} onSave={saveBlock}
          onDelete={editingBlock.id ? () => deleteBlock(editingBlock.id!) : undefined}
          onClose={() => setEditingBlock(null)} />
      )}
      {editingSession !== null && (
        <SessionModal session={editingSession} tasks={tasks} onSave={saveSession}
          onDelete={(editingSession as StudySession).id ? () => deleteSession((editingSession as StudySession).id) : undefined}
          onClose={() => setEditingSession(null)} />
      )}
      {showPrefs && (
        <PrefsModal prefs={prefs} onSave={p => { setPrefs(p); savePrefs(p); setShowPrefs(false) }} onClose={() => setShowPrefs(false)} />
      )}
      {showGenModal && (
        <GeneratePlanModal
          tasks={tasks}
          ...
          semesterStart={settings?.semester_start ?? null}
